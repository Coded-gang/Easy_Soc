...
begin
	...
	reg_i = 0;
	reg_j = 0;
	delay_lcdclk = 0; //clock ????
	count_lcd = 0;
	lcd_en = 0;	 // text-lcd device enable signal 	
end

//
// [1] text-lcd device enable signal ????
// [2] text-lcd?? ?? mode ?????? ???????? ???? counter ????
//
...
begin
	if (resetn == 0)
		begin
			reg_a <= {`lcd_y, 	`lcd_s_o, 	`lcd_s_o, 	`lcd_s_n	} ; 
	        reg_b <= {`lcd_blank, 	`lcd_s_j, 	`lcd_i, 	`lcd_blank	} ; 
	        reg_c <= {`lcd_s_h, 	`lcd_s_w, 	`lcd_s_a, 	`lcd_s_n		} ; 
	        reg_d <= {`lcd_blank, 	`lcd_s_i, 	`lcd_s_s, 	`lcd_blank		} ; 
	        reg_e <= {`lcd_s_t, 	`lcd_s_h, 	`lcd_s_e, 	`lcd_blank	} ; 
	        reg_f <= {`lcd_s_b, `lcd_s_e, `lcd_s_s, 	`lcd_s_t	} ; 
	        reg_g <= {`lcd_blank, `lcd_s_d, `lcd_s_r, `lcd_s_i	} ; 
	        reg_h <= {`lcd_s_v, `lcd_s_e, 	`lcd_s_r, 	`lcd_blank	} ; 
	        reg_i <= 32'h71982000;
